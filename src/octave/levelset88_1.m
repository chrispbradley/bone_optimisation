...
function [str,phi] = levelset88_1(nelx,nely,Vmax,tau)
...
outputarray(nely+1,nelx+1,phi,"Phi");
...
for iterNum = 1:200
  ...
  printf("\nIteration = %d\n",iterNum);
    ...
    outputindexarray(nely+1,nelx+1,2,1,U,"u")
    outputindexarray(nely+1,nelx+1,2,2,U,"v")
    
    SED = (Emin+str*(E0-Emin)).*reshape(sum((U(edofMat)*KE).*U(edofMat),2),nely,nelx);

    outputarray(nely,nelx,SED,"SED");
    
    test = reshape(sum((U(edofMat)*A).*U(edofMat),2),nely,nelx);
    TD = (1e-4+str*(1-1e-4)).*reshape(sum((U(edofMat)*A).*U(edofMat),2),nely,nelx);

    outputarray(nely,nelx,TD,"TD");
    ...
    outputarray(nely+1,nelx+1,TDN,"TDN");
    ...
    temp = C*(g2-lambda*ones(size(g2)));
    
    outputarray(nely+1,nelx+1,temp,"temp");
    
    Y = NN*(C*(g2-lambda*ones(size(g2)))+phi/dt);

    outputarray(nely+1,nelx+1,Y,"Y");
