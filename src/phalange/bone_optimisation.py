...
if len(sys.argv) > 1:
    if len(sys.argv) > 3:
        sys.exit('ERROR: too many arguments- currently only accepting up to 2 options: meshfilename bcfilename')
    meshFileName = int(sys.argv[1])
    if len(sys.argv) > 2:
        bcFileName = int(sys.argv[2])


inputFileName = meshFileName + ".mesh"
outputFileName = meshFileName + "_solution.vtk"
dirichletFileName = bcFileName + "_dirichlet_BC.npy"
neumannFileName = bcFileName + "_neumann_BC.npy"
